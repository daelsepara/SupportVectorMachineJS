	...
	.controller('SupportVectorMachineController', ['$scope', 'Webworker', 'FileSaver', 'Blob', function($scope, Webworker, FileSaver, Blob) {
			...
			if ($scope.category != 0) {
				...
				if (found < 0) {
					...
					if ($scope.SelectedKernel == $scope.KernelType.Polynomial) {
						...
						model = new ModelParameters($scope.Models.length + 1, $scope.SelectedKernel == $scope.KernelType.Sigmoid ? $scope.KernelType.Sigmoid : $scope.KernelType.Linear, [$scope.slope, $scope.intercept], $scope.regularization, $scope.passes, $scope.tolerance, $scope.category);

					} else if ($scope.SelectedKernel == $scope.KernelType.Fourier) {

						model = new ModelParameters($scope.Models.length + 1, $scope.KernelType.Fourier, [$scope.scalingFactor], $scope.regularization, $scope.passes, $scope.tolerance, $scope.category);
					}
		...
		$scope.AsyncClassifier = function() {
			...
			function async(currentPath, input, models, selected, threshold) {
				...
				var machine, item, y, p, ClassifierProgress;
		...
		$scope.RenderTestData = function() {
			...
			// function that will become a worker
			function async(currentPath, input, models, selected) {
